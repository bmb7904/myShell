 ...
 * instructions as the parent when fork was called. The philsophy of UNIX requires the OS to be 
 * composed of many smaller programs to carry out tasks. Ir a user wants to do some task, a shell allows
 * the user to specify exactly what he/she wants, and the shell will use the small, modular programs
 * in /bin to carry out the tasks. We can use fork() to
 * create child processes, and then use the exec family of functions to
 * overwrite the child's memory address with a new process. I know there are a lot of comments, probably too many, 
 * but I really don't want to forget my thought process(no pun intended) while I was writing this. 

 * Rules for my shell:

 1.) Proper spacing between commands and arguments is absolutely crucial. This shell is not smart enough to interpret what you are were "trying" to say. Improper spacing can lead to undefined behavior. 

 2.) This shell has build-in commands ("cd" and "exit"). CD will change the working directory of the current process
    calling it. It takes one argument, which is the name of the directory to be changed to. "CD .." will change the 
    working directory to that of the parent of the current working directory. 
  
 3.) If you wish to run a process in the background, append your commands with a '&' character, separated 
 by a space. this character must be the last character in your input line. You cannot run piped commands
 in the background, or with any internal process like "cd" or "exit." 
      Example: sleep 5 &

 4.) You may pipe the output of a command into another command using the '|' symbol, which will separate the two commands. 
      Example: "cat somefile.txt | more"


 5.) You may specify to the shell that you want the output of your command to be redirected to a file. This is done
  by appending your command with the ">" or ">>" commands, followed by an argument (the name of the file). Using ">" will 
  create a new file and write the output into the file instead of stdout. If a file with the specified name already exists, it will be overwritten by a new file with the same name. Using ">>" with a command (followed by an filename argument) will also create a file with the specified name and write the output to it. However, if a file with the specified name already exists, it will not be deleted, but simply appended with the redirected output. 
  
  Example: "ls -l > sample.txt" -- creates a file sample.txt and writes the output of the ls -l command to it
           If sample.txt already exists in the current working directory, it is deleted and a new file of the name
           sample.txt is created and the output written to it. 
           
           
          "ls -l >> sample2.txt" -- creates a file simple2.txt only if a file with the name sample2.txt does not already
          exists. If it does exist, the output will be appended to sample2.txt. 

 6.) You may redirect input from within this shell by using the '<' character. This only works when running a new executable. For example: writing "./a.out < input.txt" IN THIS SHELL. This shell has built in functionality to redirect stdin to a file. 

 7.) NOTE: If you wish to redirect input upon first running this shell, for example: "./a.out < input.txt" from a regular command line, you are using the linux command line's built in file redirection, not Brett's shell input redirection. This will still work, and this shell will read the commands in input.txt, but the shell will then terminate, as stdin in FD table for the parent process was overwritten and I couldn't find a way to reverse this. 
...
printf("--------------------------------------------------------------\n\n");
    ...
    int contains_pipe;
    // flag that will be flipped on if the '&' is contained within the user argument array
 ...
 * and returns the string. Will take each individual char from stdin, count each char, look for 
 * the newline '\n' char. Starts with a size of 8 bytes (each char is typcially 1 byte). If the number of
 * chars from stdin exceeds or equals the currently allocated char* buffer, use the realloc function. 
 * Doing it this way allows for dynamic reallocation  of memory, and thus, no guessing on how large 
 * user input can or will be. If an EOF is found, it means that a file was used to receive input and
 * we are at the end of the file. This occurs during input redirection from Brett's shell and also running Brett's shell 
 * like this: ./a.out < input.txt from a regular command line (not in my shell). In either case, we exit the process after
...
char* extractLine() {
    ...
    while(1) {
        ...
        // also check if c is EOF, EOF is a macro defined in stdio.h
        if (c == '\n') {
 ...
 *
 * Preconditions: The char* buffer string in parameter must be terminated with the terminating '\0' char,
 * because the strtok() function looks for it. Also, the numArgs must be correct. 
 */
char** tokenize(char* buffer, int numArgs) {
    int token_index = 0;
    ...
    // Since execvp() is expecting a null pointer at the end of the array, we can simply
 ...
 * redirected output symbol ">>" or ">". Creates memory for a fileRedir struct defined above.
 * If redirection symbols are found, information about it is stored including the index of the
 * symbol in the argArray. If no redirection symbols found, the appropirate fields of the struct 
 * are stored. Retruns a pointer to the struct. 
 */
struct fileRedir* setup_redirection(char** argArray) {
    int i = 0;
    // allocate memory for struct 
    struct fileRedir* s = malloc(sizeof(s));
    s->index = -1;
...
* This function will take the argArray with the index of the redirection symbol (redir_index), 
* which is ">" or ">>", separate out the filename to be redirected to, remove the
* redirection symbol from the argArray, create the file if it doesn't exit, or append the file
* if it does exist, and finally, overwrite the FD table of stdout(1) with the file descriptor
* of the newly created file. This function should be called after forking, and by a child process. 
* As far as I know, there is no way to reverse this, that is, put stdout back into FD 1.
*/
void redirect_output(char ** argArray, int redir_index) {
       ...
       // The following must be done after the fork to only affect child. 
            ...
            if(fr_output ->numOfSymbols == 1) {
              ...
              // for use later
              close(1);
              int output = open(fr_output->filename, O_CREAT | O_WRONLY | O_TRUNC, 0666);
              ...
              // If file exists, will return file pointer at end of file to append file
              // that's what "a" argument is for
 ...
 * the function searches and finds /bin/ls. execvp() only returns if there was an error, and 
 * it returns a -1. 
 */
...
void pipeProcesses(char** argArray, int pipe_index) {
  ...
  *(first_command + pipe_index) = NULL;
  ...
  // At this point, there are two processes, and each have their own pid variable. The child pid variable
  // is set to 0, while the parent's pid variable is set to the PID of the child. The child process
  // begins execution at this exact point where the fork was called, as the code and PC register
  // was copied exactly from the parent. We can use this to differentiate between the two processes. 
  ...
  // The parent process. 
    ...
    // The parent process now has two children, and the parent's pid and pid2 variables contain
    // the PID of its first and second child processes, respectively. 
...
/**
 * Function that takes in an array of strings, searches each string for any 
 * redirected input symbol "<" Creates memory for a fileRedir_input struct defined above.
 * If redirection symbol is found, information about it is stored including the index of the
 * symbol in the argArray. If no redirection symbols found, the appropirate fields of the struct 
 * are stored. Retruns a pointer to the struct. 
 */
struct fileRedir* setup_redirection_input(char** argArray) {
...
* This function will take the argArray with the index of the redirection symbol (redir_index), 
* which is "<", separate out the filename to be redirected to, remove the
* redirection symbol from the argArray,open the file,and
*  overwrite the FD table of stdin(0) with the file descriptor
* of the newly opened file. This function should be by the child process. 
*/
void redirect_input(char ** argArray) {

       // The following must should be done to the parent process to get input 
       // from file. But then undone 
