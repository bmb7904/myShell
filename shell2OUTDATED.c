...
void external_process(char** argArray, int bg_flag, int redir_index);
void argError();
void pipeProcesses(char** argArray, int pipe_index, int redir_index);
    ...
    while(1) {
        ...
        if( contains_cd(argArray) == 1) {
            ...
            if(pipe_index != -1) {
                ...
                // for now,
                //write(1, "\nContains pipe!\n", 16);
                pipeProcesses(argArray, pipe_index, redir_index);
            }
            else {
                external_process(argArray, bg_flag, redir_index);
            }
      }
...
void redirect_output(char ** argArray, int redir_index) {
       ...
       // The following must be done after the fork to only affect child. 
            ...
            if(strcmp(argArray[redir_index], ">") == 0) {
              ...
              // convert file pointer to file descriptor (int)
              int output = fileno(f);
    ...
    if(bg_flag == 1) {
        ...
        if(pid == 0) {
          ...
          if(redir_index != -1) {
            ...
            redirect_output(argArray, redir_index);
